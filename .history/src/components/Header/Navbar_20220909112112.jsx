...
const Navbar = ({recupereValeur}) => {
    ...
    return (
        <nav className='fixed z-10 flex justify-between items-center bg-primary w-full h-20 shadow-2xl z-1 border-b-1 border-ColorText'>
            ...
            <SiMusicbrainz size={50} style={{color:"white"}} className="hidden sm:block"/>
                <div className="hidden sm:flex">
                    ...
                    <div className='flex  gap-20 justify-between sm:flex w-[40%] bg-gray-100 rounded-full '>
                        <AiOutlineSearch size={40} className="text-gray-500 mt-1 rounded-full " />
                        <input type="text" className='w-[30%] sm:w-full h-15 bg-inherit outline-none mt-1 rounded-full text-left text-2xl shadow-2xl' onChange={recupereValeur}/>
                    </div>
                <div>
            </div>
