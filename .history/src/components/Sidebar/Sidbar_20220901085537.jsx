...
const Sidbar = () => {
  ...
  const { activeMenu, setActiveMenu, screensize,mu } = useContext(StateContext)
  return (
    <div className="flex">
        <div className={` ${open ? 'w-72':'w-20'} duration-300 bg-primary h-screen mt-20 relative flex flex-col gap-7`}>
            ...
            {menu.map((item, index) => (
                        <div className="flex gap-5 ml-5 mr-5 rounded mt-18 space-x-2 " key={index}>
                        {<span className={`text-5xl text-gray-200 ${open &&"rotate-[360deg] hover:scale-105"}`}>{item.icon}</span>}
