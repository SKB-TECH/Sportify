...
const Sidbar = (valeur) => {
  ...
  return (
    <div className="flex">
        <div className={` ${open ? 'w-72 h-[100%]':'w-20 h-[100%]'} duration-300 bg-primary h-screen mt-20 relative flex flex-col gap-7`}>
            ...
            {menu.map((item, index) => 
                      (
                        ...
                        <button onClick={valeur} className={`text-ColorText text-2xl mt-5 font-semibold ${!open && "scale-0 duration-300"}`}>{item.name}</button>
                        </div>
                    ))}
        </div>
    </div>
