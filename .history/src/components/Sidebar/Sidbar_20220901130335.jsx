        ...
        <div className={` ${open ? 'w-72':'w-20'} duration-300 bg-primary h-screen mt-20 relative flex flex-col gap-7`}>
            ...
            {menu.map((item, index) => 
                      (
                        ...
                        <button className={`text-ColorText text-2xl mt-5 font-semibold ${!open && "scale-0 duration-300"}`} onClick={search(e.target.value)}>{item.name}</button>
                        </div>
                    ))}
        </div>
    </div>
