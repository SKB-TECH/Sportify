  ...
  return (
    <div className="flex">
        <div className={` ${open ? 'w-72':'w-20'} duration-300 bg-primary h-screen mt-20 relative flex flex-col gap-7`}>
            ...
            {menu.map((item, index) => (
                        <div className="flex gap-5 ml-5 mr-5 rounded  space-x-2 shadow-2xl hover:scale-110" key={index}>
                        {<span className={`text-5xl text-gray-200 mt-10 ${open &&"rotate-[360deg] hover:scale-105"}`}>{item.icon}</span>}
                        <button className={`text-ColorText text-2xl mt-10 font-semibold ${!open && "scale-0 duration-300"}`}>{item.name}</button>
                        </div>
                    ))}
        </div>
    </div>
