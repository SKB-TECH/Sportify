...
const Leading = () => {
    ...
    useEffect(() => {
        ...
        async(e) => {
            e.preventDefault()
            fetch(`https://api.spotify.com/v1/search?q=${searchKey}&type=${}`,
               ...
               <div className='w-full mt-20 flex-wrap flex flex-gap-5'> 
                {
                   music.map((item,index)=>(
                    ...
                    <div key={index} className='w-60 h-48 bg-gray-200 rounded-lg shadow-lg ml-5 mt-10 justify-center items-center'> 
                         <img src={item.images[1]?.url} alt="" className='text-xl w-[80%]] h-40 '/>
                         <span>{item.name} </span> 
                    </div>
                   ))
                }
