               ...
               <div className='flex ml-80 mt-20 flex-wrap w-15 h-10 gap-5 '> 
                {
                   music.map((item,index)=>(
                    <div key={index} className='bg-gray-200 rounded-lg shadow-2xl ml-5 overflo'> 
                         <img src={item.images[1]?.url} alt="" className='rounded-full text-sm' style={{fontSize:"20px"}}/>
                         <span>{item.name} </span> 
                    </div>
                   ))
                }
