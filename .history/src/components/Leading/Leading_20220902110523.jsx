...
const Leading = () => {
    ...
    const [type,setType]=useState("artist,album,track")
               ...
               <div className='flex ml-80 mt-20 flex-wrap w-50 h-10 gap-5 '> 
                {
                   music.map((item,index)=>(
                    ...
                    <div key={index} className='bg-gray-800 w-[20%] rounded h-60 shadow-2xl ml-5 overflow-hidden mt-5 p-3 hover:'> 
                         <img src={item.images[1]?.url} alt="" className='w-full h-32 object-cover text-sm' style={{fontSize:"20px"}}/>
                         <span className='text-xl'>{item.name} </span> 
                            <p className='flex gap-2'>
                                <AiOutlineLike size={25}/>
                                <span className='text-ColorText textxl'>{item.followers.total}</span>
                                
                            </p>
                            <div className='w-[100%] z-10 top-[-100px] h-56  relative opacity-0  hover:opacity-100 hover:top-[-190px] transition-all delay-150 ' >
                                <AiFillPlayCircle size={70} className='absolute top-36 left-48 text-green-500' />
                               </div>
